...
export async function GET() {
  try {
    await prisma.category.upsert({ where:{slug:'precious'}, update:{}, create:{name:'Precious Stones',slug:'precious',emoji:'??'} })
    await prisma.category.upsert({ where:{slug:'semi-precious'}, update:{}, create:{name:'Semi-Precious',slug:'semi-precious',emoji:'??'} })
    await prisma.category.upsert({ where:{slug:'rudraksha'}, update:{}, create:{name:'Rudraksha',slug:'rudraksha',emoji:'??'} })
    await prisma.category.upsert({ where:{slug:'jewellery'}, update:{}, create:{name:'Jewellery',slug:'jewellery',emoji:'??'} })
    await prisma.category.upsert({ where:{slug:'organic'}, update:{}, create:{name:'Organic Gems',slug:'organic',emoji:'??'} })
    ...
    await prisma.product.createMany({
      data: [
        ...
        {name:"Cat's Eye (Lehsunia)",slug:'cats-eye',description:'Chrysoberyl cat\'s eye for Ketu. Provides protection, intuition and spiritual growth.',price:7800,weight:3.4,origin:'Sri Lanka',emoji:'Cats Eye',bgColor:'#fefce8',categoryId:precious.id,tag:'Precious',certified:true,certificate:'GII',treatment:'None',stock:3,specs:{Planet:'Ketu'}},
